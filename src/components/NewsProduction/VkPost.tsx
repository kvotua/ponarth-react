    ...
    const fetchPosts = async () => {
      try {
        const response = await axios.get(
          `https://cors-anywhere.herokuapp.com/https://api.vk.com/method/wall.get`,
          {
            params: {
              domain: groupId,
        ...
        if (
          ...
          const filteredPosts = response.data.response.items.filter(
            (post: Post) =>
              post.attachments &&
    ...
    >
      {posts.map((post) => (
        <SwiperSlide key={post.id}>
          <div
            className="vk-post"
            onClick={() => handlePostClick(post.id, post.owner_id)}
          >
            {post.attachments &&
              post.attachments.map((attachment, index) => (
                <div key={index}>
                  {attachment.type === "photo" && attachment.photo && (
                    <img
                      src={getLargestPhotoUrl(attachment.photo.sizes)}
                      alt="Post atfdsfsdfsdfhsdhfsfhiusdtachment"
